...
# Check: ######################################################################
#
#   Was cardinal_cpu_warpx.profile sourced and configured correctly?
if [ -z ${proj-} ]; then
  echo "WARNING: The 'proj' variable is not yet set in your cardinal_cpu_warpx.profile file! Please edit its line 2 to continue!"
  exit 1
fi
...
rm -rf ${build_dir}/blaspp-cardinal-cpu-build
CXX=$(which CC) cmake -S ${SRC_DIR}/blaspp \
  -B ${build_dir}/blaspp-cardinal-cpu-build \
  -Duse_openmp=ON \
  -Dgpu_backend=OFF \
  -DCMAKE_CXX_STANDARD=17 \
  -DCMAKE_INSTALL_PREFIX=${SW_DIR}/blaspp-2024.05.31
cmake --build ${build_dir}/blaspp-cardinal-cpu-build --target install --parallel 16
rm -rf ${build_dir}/blaspp-cardinal-cpu-build
